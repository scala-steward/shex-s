...
object Main
  ...
  lazy val schemaMappingCommand: Opts[SchemaMapping] =
    Opts.subcommand("mapping", "Convert a schema through a mapping") {
      (SchemaSpec.schemaSpec, mappingOpt, OutputOpt.outputOpt, VerboseLevelOpt.verboseLevel).mapN(
        SchemaMapping.apply
      )
    }
  ...
  override def main: Opts[IO[ExitCode]] = {
    ...
    schemaMappingCommand
      ...
      .orElse(Manifest.manifestCommand)
      .orElse(WikibaseValidate.wikibaseCommand)
      .orElse(SchemaCommand.schemaCommand)
      .map {
  ...
  def doSchemaMapping(smc: SchemaMapping): IO[ExitCode] = for {
    ...
    newSchema <- mapping
      ...
      .fold(
        ...
        s => s.pure[IO],
        (warnings: List[ProcessingError], s: Schema) =>
          for {
  ...
  def doShapePathEval(spc: ShapePathEval): IO[ExitCode] = for {
    ...
    shapePath <- IO.fromEither(
      ShapePath
        .fromString(spc.shapePath, "Compact", None, schema.prefixMap)
        .leftMap(err => new RuntimeException(s"Error parsing shapePath: ${err}"))
    )
    result <- {
