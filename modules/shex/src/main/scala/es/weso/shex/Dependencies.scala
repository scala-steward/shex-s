...
//    println(s"Adding edge $d to graph: $g")
      ...
      case s: Shape =>
        for {
          depsExtras <- s.extra match {
            ...
            case Some(es) =>
              s.expression
                .map(te => dependenciesExtras(es, schema, source, te, posNeg))
                .getOrElse(noDeps)
          }
          depsTripleExpr <- s.expression
            .map((tripleExpr: TripleExpr) =>
              dependenciesTripleExpr(schema, source, tripleExpr, posNeg)
            )
            .getOrElse(noDeps)
  ...
  def dependenciesExtras(
    ...
    es.map(dependenciesExtra(_, schema, source, tripleExpr, posNeg))
      .sequence[ES, Deps]
      .map(_.flatten)
